...
angular.module('blogControllers', [])
  ...
  .controller('blogViewCtrl', ['$scope', '$rootScope', '$routeParams', 'myApi', function ($scope, $rootScope, $routeParams, myApi) {
    ...
    $scope.limit = 10;
    // $scope.comments = [];
    myApi.get('blog/' + blogId).then(function (response) {
      $scope.blogEntry = response.blog;
      if ($scope.blogEntry) {
          $rootScope.make_listing('comments/' + blogId + '?limit=' + $scope.limit);
      }
    });
    // scroll function
    ...
    $scope.comment = function () {
      ...
      myApi.post('comment/' + blogId, form_data).then(function (response) {
        $('#comment_text').val('')
        $rootScope.make_listing('comments/' + blogId + '?limit=' + $scope.limit);
      });
    }
